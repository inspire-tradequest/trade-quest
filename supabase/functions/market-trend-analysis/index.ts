...
serve(async (req) => {
  ...
  try {
    ...
    const { assets, timeframe, indicators } = requestData;
    ...
    const { data: logData, error: logError } = await supabaseClient
      ...
      .insert({
        user_id: user.id,
        request_type: 'market_trend_analysis',
    ...
    const aiResponse = await fetch(`${AI_SERVICE_URL}/market-trends`, {
      method: 'POST',
      headers: {
    ...
    if (saveError) {
      console.error('Error saving trend analysis:', saveError);
    }
    ...
    console.error('Error in market-trend-analysis function:', error);
